...
class DefaultServer
{
    ...
    public static function getInstance(): DefaultServer
    {
        if (self::$instance === null) {
            self::$instance = new DefaultServer();
        }
...
//        $port = 9999;
    ...
    {
        Log::channel('analyzer_communication')->error('Waiting for client to reconnect at ' . now());
    ...
    private function saveResult(string $inc): bool
    {
        ...
        if (!$order) {
            Log::channel('analyzer_communication')->error('Order not found for barcode: ' . $this->barcode);
            return false;
        }
